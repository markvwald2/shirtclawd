...
def approve_post(post, source_file, handle, path=DEFAULT_APPROVAL_QUEUE_PATH):
...
def is_post_approved(post, source_file, handle=None, path=DEFAULT_APPROVAL_QUEUE_PATH):
    queue = load_approval_queue(path)
    for item in queue["approved_posts"]:
