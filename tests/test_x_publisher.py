...
from bot.x_publisher import build_x_status, load_posts, publish_post, select_post
...
class XPublisherTests(unittest.TestCase):
    ...
    def test_build_x_status_trims_long_caption(self):
        post = {
            "title": "Long Post",
        ...
        status = build_x_status(post)
        self.assertLessEqual(len(status), 280)
    ...
    def test_publish_post_dry_run_logs_event(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = publish_post(post, dry_run=True, log_path=log_path)

            self.assertEqual(result["mode"], "dry_run")
            self.assertEqual(result["handle"], "@3rdStringShirts")
